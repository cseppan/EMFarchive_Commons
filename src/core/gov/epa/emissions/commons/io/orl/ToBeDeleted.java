...
public class ORLImporter {
    ...
    private void doImport(File file, Dataset dataset, String table, FileFormatWithOptionalCols fileFormat,
        ...
        File headerFile = new File("/home/qhe/temp/.header");
        File dataFile = new File("/home/qhe/temp/.data");
        
        splitFile(file, headerFile.getAbsolutePath(), dataFile.getAbsolutePath());
        loadDataset(getComments(headerFile), dataset);

        String copyString = "COPY " + getFullTableName(table) + " " + getColNames(fileFormat) + " FROM '" + dataFile.getAbsolutePath()
                + "' WITH CSV ";
...
//        dataFile.delete();
    ...
    private void splitFile(File file, String headerFile, String dataFile) throws IOException, InterruptedException {
        ...
        String createHeaderFile = "grep \"^#\" " + file.getAbsolutePath() + " > " + headerFile;
        String createDataFile = "grep -v \"^#\" " + file.getAbsolutePath() + " > " + dataFile;
        
        cmd = new String[] { "sh", "-c", createHeaderFile + ";" +  createDataFile};

        Process p = Runtime.getRuntime().exec(cmd);
        p.waitFor();
    }

    private List getComments(File file) throws Exception {
    ...
    private String getColNames(FileFormatWithOptionalCols fileFormat) {
