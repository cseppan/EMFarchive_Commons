...
public class IDAAreaColumnsMetadata implements ColumnsMetadata {
	...
	public IDAAreaColumnsMetadata(SqlDataTypes dataTypes) {
		String intType = dataTypes.intType();
		colTypes = new String[] { intType, intType, dataTypes.stringType(10) };
		colNames = new String[] { "STID", "CYID", "SCC" };
		widths = new int[] { 2, 3, 10 };
	}
	...
	private int[] addPollWidths(int length, int[] desColWidths) {
		...
		for (int i = 0; i < length; i ++) {
			int startIndex = desColWidths.length+i*6;
