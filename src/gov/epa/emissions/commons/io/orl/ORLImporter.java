...
public class ORLImporter implements Importer {
	...
	public void run(Dataset dataset) throws ImporterException {
		...
		try {
			...
			throw new ImporterException("could not create table for dataset - "
					+ dataset.getName(), e);
		}
	...
	private void dropTable(String table, Datasource datasource)
		...
		try {
			...
			throw new ImporterException("could not drop table " + table
					+ " after encountering error importing dataset", e);
		}
	}
	...
	private void addAttributesExtractedFromComments(List comments,
			Dataset dataset) throws ImporterException {
		if (!(tag("#ORL", comments) != null))
			throw new ImporterException(
					"The tag - 'ORL' is mandatory, and is invalid");

		String country = tag("#COUNTRY", comments);
		if (country == null || country.length() == 0)
			throw new ImporterException(
					"The tag - 'COUNTRY' is mandatory, and is invalid");
		...
		if (year == null || year.length() == 0)
			throw new ImporterException(
					"The tag - 'YEAR' is mandatory, and is invalid");
