...
public class NIFOnRoadSummary implements SummaryTable {
	...
	public void createSummary() throws Exception {
        ...
        for (int i = 0; i < numOfPollutants; i++) {
            ...
            joinPart = joinPart + "LEFT JOIN (SELECT " + "state_county_fips" + ", " + "scc" + ", " + "emission_value" + " FROM "
                    + emTable + " WHERE " + "pollutant_code" + " = '" + pollutants[i] + "') " + pollutant + " ON (e."
                    + "state_county_fips" + " = " + pollutant + "." + "state_county_fips" + " AND e." + "scc" + " = " + pollutant + "."
                    + "scc" + ") ";
            rs.next();
        }
        rs.close();
        ...
        String query = "CREATE TABLE " + summaryTable + " AS SELECT DISTINCT f." + "state_abbr" + " as " + "State" + ", "
                ...
                + ", " + selectPart + " FROM " + peTable + " as pe, " + referenceDatasource.getName()
                + ".fips as f, " + "(SELECT DISTINCT " + "state_county_fips" + ", " + "scc" + " FROM " + emTable + ") e "
